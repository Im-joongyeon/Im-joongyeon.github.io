...
---------------------------------------------------------        
[schema.yml] // 테이블 스키마 정의 및 테스트 코드 작성         
models:
  - name: daily_dex_volume
    tests:
      - unique:
          column_name: "concat(date, protocol, chain)"
      - not_null:
          column_name: "volume_usd"
    columns:
      - name: volume_usd
        tests:
...
```
with DAG('dbt_daily_metrics') as dag:
    ...
    run_staging = BashOperator(
        task_id='run_staging',
        bash_command='dbt run --select tag:staging'
    )
    
    # 3. marts 모델 실행
    run_marts = BashOperator(
        task_id='run_marts', 
        bash_command='dbt run --select tag:marts'
    )
    
    freshness_check >> run_staging >> run_marts
```
